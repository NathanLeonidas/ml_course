...
def get_dataloaders(data_config, use_cuda):
    ...
    base_dataset = torchvision.datasets.Caltech101(root=root_dir, transform=None, download=False)
    ...
    logging.info(f"  - I Created dataset {train_dataset}")
    logging.info(f"  - I Created dataset {valid_dataset}")
    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    augmentation_transforms = [v2.RandomHorizontalFlip(p=0.5),
    ...
    logging.info(f"  - I Created dataloader {train_loader}")
    logging.info(f"  - I Created dataloader {valid_loader}")
    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
